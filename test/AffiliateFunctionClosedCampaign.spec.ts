...
import { loadCampaignCreatedEvent, loadAffiliateAskToJoinAllowedListEvent, loadAdvertiserModifiedAllowedListEvent } from '../scripts/events'; // Ensure this utility is correctly set up for testing
...
describe('Affiliate Actions - Positive and Negative Tests for Affiliate Functions', () => {
    ...
    it('should allow affiliate to ask to join a campaign and allow advertiser to approve', async () => {
		...
		let decodedAdvertiserModifiedAllowedList: any | null = null;
        for (const external of advetiserApprovesRequestResult.externals) {
            if (external.body) {
                decodedAdvertiserModifiedAllowedList = loadAdvertiserModifiedAllowedListEvent(external.body);
            }
        }
		
		expect(decodedAdvertiserModifiedAllowedList).not.toBeNull();
		expect(decodedAdvertiserModifiedAllowedList!.isAdded).toBe(true);
