...
/**
 * Retrieve only unread notifications (i.e., readAt IS NULL) for a user.
 */
export async function getUnreadNotificationsForUser(userId: number, campaignId: string): Promise<Notification[]> {
  try {
    return await notificationRepository().find({
      where: {
        userId,
        campaignId,
        readAt: IsNull(), // filter unread
      },
      order: { createdAt: 'DESC' },
    });
  } catch (err) {
    Logger.error(`Error fetching unread notifications for user ${userId}: ` + err);
