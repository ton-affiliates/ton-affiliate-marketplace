...
export async function run(provider: NetworkProvider, args: string[]) {
	...
	let tonToSend = toNano(BigInt(args.length > 2 ? args[2] : await ui.input('Amount of TON to replenish for gas: (e.g. 3) ')));
	
	const campaign = provider.open(Campaign.fromAddress(campaignAddress));
	let numAdvertiserReplenishBefore = (await campaign.getCampaignData()).numAdvertiserReplenishGasFees;
    ...
    ui.write('Waiting for campaign to update numAdvertiserReplenishGasFees...');
	
	let numAdvertiserReplenishAfter = (await campaign.getCampaignData()).numAdvertiserReplenishGasFees;
    let attempt = 1;
    ...
    while (numAdvertiserReplenishBefore == numAdvertiserReplenishAfter) {
        ...
        await sleep(2000);
		numAdvertiserReplenishAfter = (await campaign.getCampaignData()).numAdvertiserReplenishGasFees;
        attempt++;
    }
